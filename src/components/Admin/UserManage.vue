      ...
      <table class="bg-indigo-400 text-white">
        ...
        <tbody class="box">
          <tr v-for="user in allUser" :key="user.id">
            ...
            <td>{{ user.name }}</td>
            <td>{{ user.status }}</td>
            <button class="bg-red-800" @click="banUser(user.id , user.name) "  v-if="user.status==='ACTIVE'"> 
              BAN
            </button>
            <button class="bg-green-500" @click="activeUser(user.id , user.name)" v-if="user.status==='BANNED'"> 
              ACTIVE
            </button>
          </tr>
        </tbody>
     ...
     async activeUser(isid , name) {
      ...
      console.log(isid);
       swal({
            ...
            await AuthUser.dispatch("activeUser", isid);
