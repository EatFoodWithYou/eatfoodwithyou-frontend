<template>
  <div>
    ...
    <div class="head">
        ...
        <img class="h-48 w-full object-cover md:w-48" v-bind:src="this.currentFood.photo_url" alt="Man looking at item at a store">
    ...
    <div class="Ingredient">
        ...
        <tbody>
            <tr v-for="ingredient,index in currentFood.ingredients" :key="index">
                <td> {{ index+1}} </td>
    ...
    <div class="cookingProcess">
        <h1>ขั้นตอนการทำ </h1>
                <div v-for="(process, index) in currentFood.cooking_processes" :key="index">
                    <h2>{{index+1}}</h2>
                    <h2>{{process.process}}</h2>
                    <td v-if="process.photo!==null"><img class="h-48 w-full object-cover md:w-48" v-bind:src="`http://localhost:8000/storage/cookingProcess/${process.photo}`"></td>
