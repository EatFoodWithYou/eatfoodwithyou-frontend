<template>
  <div class="container">
    ...
    <div class="p-8">
      <router-link :to="{ name: 'FoodRecipeInformation', params: { id: this.foodRecipe.id}}" class="block mt-1 text-lg leading-tight font-medium text-black hover:underline">{{ this.foodRecipe.name }}</router-link>
      ...
      <div>
      <button @click="fetchRandomFoodRecipes" class="object object-right">Refresh</button>
      <div class="flex flex-warp bg-white rounded-xl shadow-md overflow-hidden">
      <food-recipe-item
  ...
  components: { foodRecipeItem },
    ...
    methods:{
        async fetchRandomFoodRecipes(){
            let res = await axios.get('http://localhost:8000/api/recipes/randoms');
            // console.log(res);
            this.randomFoodRecipe = res.data.data;
            // console.log(this.randomFoodRecipe);
        },
        async fetchRandomFoodRecipe(){
            let res = await axios.get('http://localhost:8000/api/recipes/random');
            // console.log(res);
            this.foodRecipe = res.data.data;
            if(this.foodRecipe.photo_url === 'http://localhost:8000/storage/foodRecipe/')
                this.foodRecipe.photo_url = 'https://via.placeholder.com/250x250'
            // console.log(this.randomFoodRecipe);
        }
    },
    async created(){
